...
| L04 Drafts/scripts | Edit while state echoes arrive, save and keep typing, provoke a rejected path, interrupt the connection during a request, then close/New/Reload/quit with a dirty buffer. | The submitted revision alone becomes saved; newer typing survives; errors/timeouts become visible; dirty work requires an explicit discard. Tagged stale replies cannot complete another request (also covered automatically). |
| L05 Profiles/layouts | Create/switch/rename/delete a profile; change the selection during editing; import/export a temporary layout and try changes during backend freeze. | Correct profile is targeted, duplicates/stale names are rejected, the last profile is preserved; layout roundtrip is coherent and forbidden edits are refused. Record slow-dialog/file freezes separately. |
...
| L08 Overlay/SOFA | Change display preferences with an mpv fixture; browse/import a temporary SOFA file, including error and cancellation paths. | Changed overlay preferences synchronize once; unrelated state creates no repeated traffic. SOFA failures leave a usable UI and report the reason. |
