...
pub fn control_diag_rate_hz(state: &SharedState, value: f32) {
    let clamped = value.max(1.0).min(1000.0);
    send_control(
        &state.osc_tx,
        OscControlMsg::SendFloat {
...
pub fn control_diag_publication_enabled(state: &SharedState, enable: i32) {
    send_control(
        &state.osc_tx,
...
pub fn subscribe_speaker_gaintable(state: &SharedState, have_version: i32, speaker_index: i32) {
    send_control(
        &state.osc_tx,
...
pub fn unsubscribe_speaker_gaintable(state: &SharedState) {
    send_control(
        &state.osc_tx,
...
pub fn debug_state_sizes(state: &SharedState) -> serde_json::Value {
...
pub fn debug_write_memory_csv(app: &HostPaths, csv: String) -> Result<String, String> {
    let dir = app
        .download_dir
        .clone()
        .or_else(|| app.log_dir.clone())
        .ok_or_else(|| "no download directory".to_owned())?;
