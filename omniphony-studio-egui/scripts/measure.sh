...
#
# Linux /proc measurement. Output and an isolated configuration go into a new
# temporary directory (or STUDIO_MEASURE_DIR); the user's preferences are untouched.
# STUDIO_BINARY can select an installed executable. No desktop capture is made.
set -euo pipefail
...
CRATE="$(cd "$HERE/.." && pwd)"
BIN="${STUDIO_BINARY:-$CRATE/target/release/omniphony-studio-egui}"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUTPUT="${STUDIO_MEASURE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/studio-measure.XXXXXX")}"
mkdir -p "$OUTPUT"
OUTPUT="$(realpath "$OUTPUT")"
LOG="$OUTPUT/measure-$STAMP.log"
export OMNIPHONY_CONFIG_DIR
OMNIPHONY_CONFIG_DIR="$(mktemp -d "$OUTPUT/config.XXXXXX")"
CLK="$(getconf CLK_TCK)"
...
{
  echo "objects=$OBJECTS rate_hz=$RATE feed_secs=$FEED_SECS extra_flags=${*:4}"
  echo "binary=$BIN config=$OMNIPHONY_CONFIG_DIR"
