...
export function renderAdaptiveResamplingUI() {
  ...
  if (adaptiveKpNearInputEl && !app.adaptiveKpNearEditing && !app.adaptiveKpNearDirty) {
    adaptiveKpNearInputEl.value = app.adaptiveResamplingKpNear === null ? '' : Number(app.adaptiveResamplingKpNear).toFixed(8);
  }
  if (adaptiveKpFarInputEl && !app.adaptiveKpFarEditing && !app.adaptiveKpFarDirty) {
    adaptiveKpFarInputEl.value = app.adaptiveResamplingKpFar === null ? '' : Number(app.adaptiveResamplingKpFar).toFixed(8);
  }
  if (adaptiveKiInputEl && !app.adaptiveKiEditing && !app.adaptiveKiDirty) {
    adaptiveKiInputEl.value = app.adaptiveResamplingKi === null ? '' : Number(app.adaptiveResamplingKi).toFixed(8);
  }
  if (adaptiveMaxAdjustInputEl && !app.adaptiveMaxAdjustEditing && !app.adaptiveMaxAdjustDirty) {
    adaptiveMaxAdjustInputEl.value = app.adaptiveResamplingMaxAdjust === null ? '' : Number(app.adaptiveResamplingMaxAdjust).toFixed(6);
  }
  if (adaptiveMaxAdjustFarInputEl && !app.adaptiveMaxAdjustFarEditing && !app.adaptiveMaxAdjustFarDirty) {
    adaptiveMaxAdjustFarInputEl.value = app.adaptiveResamplingMaxAdjustFar === null ? '' : Number(app.adaptiveResamplingMaxAdjustFar).toFixed(6);
  }
  ...
  if (adaptiveMeasurementSmoothingAlphaInputEl && !app.adaptiveMeasurementSmoothingAlphaEditing && !app.adaptiveMeasurementSmoothingAlphaDirty) {
    adaptiveMeasurementSmoothingAlphaInputEl.value =
      app.adaptiveResamplingMeasurementSmoothingAlpha === null ? '' : Number(app.adaptiveResamplingMeasurementSmoothingAlpha).toFixed(2);
  }
...
export function resetAdaptiveResamplingAdvancedDirtyState() {
  ...
  app.adaptiveKpFarDirty = false;
  app.adaptiveKpFarEditing = false;
  app.adaptiveKiDirty = false;
  app.adaptiveKiEditing = false;
  app.adaptiveMaxAdjustDirty = false;
  app.adaptiveMaxAdjustEditing = false;
  app.adaptiveMaxAdjustFarDirty = false;
  app.adaptiveMaxAdjustFarEditing = false;
  ...
  app.adaptiveMeasurementSmoothingAlphaDirty = false;
  app.adaptiveMeasurementSmoothingAlphaEditing = false;
