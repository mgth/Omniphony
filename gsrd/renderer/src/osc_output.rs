...
impl OscSender {
    ...
    pub fn send_meter_bundle(
        ...
        if let Some(ref control) = self.control {
            ...
            messages.push(OscPacket::Message(OscMessage {
                addr: "/gsrd/state/audio/output_device".to_string(),
        ...
        let bundle = OscPacket::Bundle(OscBundle {
            ...
            content: vec![
                OscPacket::Message(OscMessage {
                    addr: "/gsrd/state/audio/output_devices".to_string(),
...
fn handle_control_message(
    ...
    if addr == "/gsrd/control/audio/output_device" {
        ...
        broadcast_string(
            socket,
            clients,
            "/gsrd/state/audio/output_device",
            requested.as_deref().unwrap_or(""),
        );
        return;
    }

    if addr == "/gsrd/control/ramp_mode" {
        ...
        set_dirty(control, socket, clients);
        broadcast_string(socket, clients, "/gsrd/state/ramp_mode", mode.as_str());
    ...
    // ── Cartesian VBAP table sizes: /gsrd/control/vbap/cart/{x_size|y_size|z_size|z_neg_size} ──
        ...
        if let Some(size) = size {
            let state_addr = match rest {
                ...
                "z_neg_size" => {
                    ...
                    Some("/gsrd/state/vbap/cart/z_neg_size")
                }
                _ => None,
            };
    ...
    // ── Room lower ratio: /gsrd/control/room_ratio_lower r ──────────────────
    if addr == "/gsrd/control/room_ratio_lower" {
        if let Some(OscType::Float(f)) = msg.args.first() {
            let v = f.max(0.01);
            control.live.write().unwrap().room_ratio_lower = v;
            set_dirty(control, socket, clients);
            broadcast_float(socket, clients, "/gsrd/state/room_ratio_lower", v);
...
fn build_live_state_bundle(control: &Arc<RendererControl>) -> Vec<u8> {
    ...
    let mut messages = vec![
        ...
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/vbap/cart/z_neg_size".to_string(),
        ...
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/log_level".to_string(),
            args: vec![OscType::String(
                sys::live_log::current_runtime_level_name().to_string(),
            )],
        }),
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/ramp_mode".to_string(),
        ...
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/room_ratio_lower".to_string(),
...
fn send_metering_state(socket: &UdpSocket, client: SocketAddr, enabled: bool) {
    let packet = OscPacket::Message(OscMessage {
        addr: "/gsrd/state/osc/metering".to_string(),
